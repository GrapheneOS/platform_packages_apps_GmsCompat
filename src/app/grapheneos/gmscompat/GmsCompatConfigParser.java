...
public class GmsCompatConfigParser {
    ...
    public static ApplicationInfo configHolderInfo(Context ctx) throws PackageManager.NameNotFoundException {
        ...
        if (Build.isDebuggable()) {
            try {
                return pm.getApplicationInfo(ConfigUpdateReceiver.CONFIG_HOLDER_PACKAGE_DEV, 0);
            } catch (PackageManager.NameNotFoundException e) {
                // fallthrouugh
            }
        }
        return pm.getApplicationInfo(ConfigUpdateReceiver.CONFIG_HOLDER_PACKAGE, 0);
    }
    ...
    private static GmsCompatConfig execInner(Context ctx, String pkg) throws PackageManager.NameNotFoundException, IOException {
        ...
        try (ZipFile f = new ZipFile(path)) {
            ...
            try (InputStream inputStream = f.getInputStream(e)) {
                configBytes = readAllBytes(inputStream);
            }
        }
    ...
    // copied from AOSP T, libcore/ojluni/src/main/java/java/io/InputStream.java
    public static byte[] readAllBytes(InputStream stream) throws IOException {
        return readNBytes(stream, Integer.MAX_VALUE);
    }

    private static final int DEFAULT_BUFFER_SIZE = 3 * 4096;
